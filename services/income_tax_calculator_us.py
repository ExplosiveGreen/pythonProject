...
def main(income):
    income=float(income)
    tax,remain=income_tax_calculetor(income)
    result =  {
        "total tax":number_butifier(tax),
        "income after tax":number_butifier(remain),
        "tax rate":f'{number_butifier(tax/income*100.0)}%'
    }
    print(result)
